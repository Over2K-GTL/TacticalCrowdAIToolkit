...
public:
	/** Primary layers where influence components or sources directly emit data.
	 * Examples: Enemy, Ally, Fire, etc.
	 */
	UPROPERTY(Config, EditAnywhere, Category = "Layer Setup")
	TSet<FName> BaseInfluenceTags;

	/** Derived layers generated by combining or performing operations on other layers.
...
public:
	...
	/** Multiplier to add a safety margin when comparing GPU and CPU update times.
	 * A value > 1.0 means the CPU update time must be significantly lower than the GPU time to switch to CPU mode.
	 */
	UPROPERTY(Config, EditAnywhere, Category = "Advanced|Adaptive GPU/CPU update mode switching")
	float ModeSwitchingSafetyMultiplier = 1.0f;
	...
	UPROPERTY(Config, EditAnywhere, Category = "Advanced|Adaptive GPU/CPU update mode switching")
	float WaitTimeMsThresholdForGPUMode = 2.5f;
